...
/*
	...
	simulate the fetch/parse process for louisville

	use the json_parse logic to simulate the packets for each fetch/parse level

	for the fetch level, save the file, use the file to be sent 
	 to the parse process...
	don't use the queues..
	don't use the actual tbls..

	simulate going through one/complete branch... 
	create required packet/fetchparseurlpacket as well
	modify packet as output, to be the input of the 
	 next process...
	create basic parentApps to call the fetch/parse apps..

	the goal of all of this is to have a basic/good process that 
	 can be used to hook into the crawling logi/processes

*/


/*
	test_server_ping_inc.php

	include file/function to be called by parent app/function to 
	see if the webserver is running.. without a running webserver, 
	the gearman/parse/fetch processes will crash...
	
	php parent app/functions includes this file/function which is 
	is used to always quickly check/wait until the webserver is 
	up running

	//--should probably do an email/alert of some sort to notify some service/system 
		that the function isn't able to detect the remote webserver..
...
function testServerPing()
{
	...
	while($a) 
	{
		$t = "curl -s -o /dev/null http://192.168.5.141 2>&1";
		//$t=system($t);
		$t=system($t);
		//if (strpos($t,"Destination Host Unreachable")>0)
		if (strpos($t,"couldn't connect to host")>0)
		{
			//--sleep, and try again
			sleep(5);
		}
...
/*
	used to generate a random id on a linux system..
		replaces the "-" with "_"
		eliminates the pdo db error from the "-" in the id
*/
function uuidgen()
{
	//$id="uuidgen -t > /dev/null";
	$id=`uuidgen -t`;
...
/*
	used to generate a random user_agent for use in the fetch 
	process..
*/
function gen_useragent()
{
	...
	$ua = array (
            ...
            "Operagen_useragent()/9.80 (Windows NT 6.1; U; en) Presto/2.6.30 Version/10.61",
